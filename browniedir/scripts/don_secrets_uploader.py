...
from scripts.secrets_response_utils import (
...
def random_message_id_uint32_str() -> str:
    ...
    # body.message_id = `${Math.floor(Math.random() * Math.pow(2, 32))}`
...
def normalize_encrypted_secrets_hex(value: str, *, base_dir: Optional[Path] = None) -> str:
    ...
    - path to a file containing the hex string (relative to base_dir if provided)
    """
...
def build_secrets_payload(
    *,
    ...
    """
    Builds the payload required for gateway method `secrets_set`.
    Mirrors toolkit semantics:
    - version = floor(Date.now()/1000) unless overridden
    - expiration is unix milliseconds now + minutes_until_expiration unless overridden
    - signature = EIP-191 signature of JSON.stringify(storage_message) (no spaces)
    """
...
def build_gateway_request_json(
    *,
    ...
    """
    Returns (request_json_string, debug_info).
    request_json_string is EXACT JSON string to send as HTTP body (mirrors axios.post(url, JSON_STRING)).
    """
    body = {
...
def format_curl_command(*, gateway_url: str, request_json: str) -> str:
    """
    Returns a shell-safe curl command using heredoc, so you can copy/paste it.
    """
    return (
        "curl -sS -X POST "
...
def upload_encrypted_secrets_to_don(
    *,
    ...
    """
    Full utility to upload encrypted secrets to DON-hosted storage via Functions gateways.

    Returns a dict compatible with toolkit semantics:
      { "version": <int>, "success": <bool>, "gatewayUrl": <str>, "nodeResponses": [...] }

    Gateway selection mirrors toolkit behavior:
    - Iterate gateways until one returns gateway-level payload.success==true.
    - Then evaluate node responses:
      - if ALL nodes failed -> raise
      - else -> return summary (success true only if 0 node failures)
    """
    ...
    # Optional debug: verify storage signature recovers to signer address.
    if debug:
