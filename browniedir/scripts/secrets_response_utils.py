...
def normalize_secrets_set_gateway_response(*, gateway_url: str, gateway_json: Dict[str, Any]) -> SecretsSetGatewayResponse:
        ...
        "result": {
          "body": {
            "payload": {
              "success": true|false,
              "node_responses": [
                { "body": { "payload": { "success": true|false, "error_message": "..." } } },
                ...
              ]
            }
          }
    ...
    If the response is a JSON-RPC error, raise ValueError.
    """
    ...
    for entry in raw_node_responses:
        # Toolkit reads: r.body.payload.success and optionally error_message.
        try:
...
def summarize_upload_encrypted_secrets_result(
    *,
    ...
    - If SOME nodes failed -> return success=false (but caller may still proceed with this version)
